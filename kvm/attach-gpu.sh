#!/usr/bin/env bash
###############################################################################
# GPU 패스스루 attach 스크립트
# - VM 을 종료한 뒤 지정한 bus 의 PCI 장치(function 0, 1)를 hostdev 로 추가
# - 반영 후 VM 재시작
#
# 실행 방법: bus=<bus> bash attach-gpu.sh <VM이름>
#   예: bus=3b ./attach-gpu.sh myvm   (bus 값은 gpu-list.sh 로 확인)
###############################################################################
...
step_header "GPU attach 시작 (${VM} ← bus=${bus})"
...
step_header "Step 2/3: hostdev attach (function 0, 1)"
# 3) hostdev attach (function 0, 1)
for func in 0 1; do
    log_info "attach ${bus}:00.${func} ..."
    virsh attach-device "$VM" /dev/stdin --config <<HOSTDEV
...
log_success "${bus}:00.0, ${bus}:00.1 반영 후 ${VM} 재시작했습니다."
